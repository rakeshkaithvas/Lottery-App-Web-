...
@section('title', 'Update Gateways')

@include('Admin.partials.alerts.errors')

@include('Admin.partials.alerts.success')

<form action="{{ route('update.gateway', ['id' => $gateway->id]) }}" method="post" enctype="multipart/form-data" id="gatewayForm">
    @csrf

    @if(count($gateway->data) > 0)
    <div class="col-xl-12">
        <div class="card custom-card">
            ...
            @foreach ($gateway->data->chunk(3) as $fieldsChunk)
            <div class="row mb-none-15 mb-3">
                @foreach ($fieldsChunk as $field)
                <div class="col-sm-12 col-md-6 col-lg-6 col-xl-4 mb-15">
                    <div class="form-group mb-2">
                        <label for="{{ $field->field_name }}" class="required">{{ ucwords(str_replace('_', ' ', $field->field_name)) }} <span
                            class="small text-danger">*</span></label>
                        @if ($field->field_name === 'isSandbox' && ($field->field_value === '1' || $field->field_value === '0'))
                        <select class="form-control" name="gateway_data[{{ $field->id }}]" id="isSandbox" required="">
                            <option value="1" {{ $field->field_value == 1 ? 'selected' : '' }}>Yes</option>
                                <option value="0" {{ $field->field_value == 0 ? 'selected' : '' }}>No</option>
                            </select>
                        @else
                        <input type="text" class="form-control" name="gateway_data[{{ $field->id }}]" value="{{ $field->field_value }}" required="">
                        @endif
                    </div>
                </div>
    ...
    <div class="col-xl-12">
        <div class="card custom-card">
            ...
            <div style="margin-left: 1rem; margin-top: 1rem; margin-right: 1rem">
                    ...
                    <div class="col-sm-12 col-md-6 col-lg-6 col-xl-4 mb-15">
                        <div class="form-group mb-2">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="any" class="form-control" name="rate" required="" value="{{ $gateway->rate }}"
                                    id="rate">
                ...
                <div class="row mb-3">
                    <div class="col-xl-4">
                        <label for="input-rounded" class="form-label">Minimun Deposit<span
                                class="small text-danger">*</span></label>
                        <div class="input-group">
                            <input class="form-control" type="number" name="min" id="" required value="{{ $gateway->min }}">
                            <div class="input-group-text"><span class="currency_symbol" id="currencyData2">{{ $gateway->currency }}</span></div>
                        </div>
                    </div>
                    <div class="col-xl-4">
                        <label for="input-rounded" class="form-label">Maximum Deposit<span
                                class="small text-danger">*</span></label>
                        <div class="input-group">
                            <input class="form-control" type="number" name="max" id="" required value="{{ $gateway->max }}">
                    ...
                    <div class="col-xl-4">
                        <label for="input-rounded" class="form-label">Fee (e.g 2)<span
                                class="small text-danger">*</span></label>
                        <div class="input-group">
                            <input class="form-control" type="number" name="fee" id="" required value="{{ $gateway->fee }}">
                ...
                <div class="mb-3">
                    <div class="col-xl-12">
                        <label for="input-rounded" class="form-label">Deposit Instruction</label>
